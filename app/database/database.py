...
def get_chunk_statuses_ordered(job_id: str) -> list[tuple]:
    """Return (chunk_id, status, srt_path) for every chunk, ordered by chunk_index."""
    with get_db_session() as db:
        chunks = (
        ...
        return [(str(c.id), c.status, c.srt_path) for c in chunks]